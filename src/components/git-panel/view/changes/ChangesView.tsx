      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="mb-2 text-lg font-medium text-foreground">{t('changes.noCommitsYet')}</h3>
            <p className="mb-6 max-w-md text-sm text-muted-foreground">
              {t('changes.noCommitsYetDescription')}
            </p>
            <button
          ...
          <div className="flex h-32 flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="text-sm">{t('changes.noChangesDetected')}</p>
          </div>
        ) : (
          <div className={isMobile ? 'pb-4' : ''}>
            ...
            {selectedFiles.size === 0 ? (
              <div className="px-3 py-2 text-xs text-muted-foreground italic">{t('changes.noStagedFiles')}</div>
            ) : (
              <FileChangeList
                gitStatus={gitStatus}
                gitDiff={gitDiff}
            ...
            <div className="flex items-center justify-between border-b border-border/60 bg-muted/30 px-3 py-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                {t('changes.changes', { count: unstagedFiles.size })}
              </span>
            ...
            {unstagedFiles.size === 0 ? (
              <div className="px-3 py-2 text-xs text-muted-foreground italic">{t('changes.allChangesStaged')}</div>
            ) : (
              <FileChangeList
                gitStatus={gitStatus}
                gitDiff={gitDiff}
